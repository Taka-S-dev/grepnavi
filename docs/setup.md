...
システムトレイに常駐し、トレイメニューの「開く / 終了」でウィンドウを操作します。UI は埋め込み WebView2 で表示するため、ブラウザ拡張機能（アドオン）の影響を受けません。

ウィンドウは OS のタイトルバーを持たず、最小化 / 最大化 / 閉じるはページ右上の自前ボタンで行います（バーの空き部分をドラッグで移動、素早い2回押しで最大化）。グラフヘッダもこのバーに統合され、1行ぶん表示が広くなります。プロジェクト操作は「ファイル」メニュー、ツリーの描画切り替え（パス・メモ・グラフ）と各アドオンパネルは「表示」メニューにまとまり、ツリーの消去はタブの右クリックから行います。開いている JSON 名はステータスバー左に表示されます。従来の OS タイトルバーに戻すには環境変数 `GREPNAVI_NATIVE_TITLEBAR=1` を設定して起動してください。
...
提供しないもの: rename（検索ベースで名前を書き換えると別のシンボルまで変わるため）、診断・コードアクション・整形（コンパイラの領分のため）。

要求は同時に 4 件まで並行して処理し、エディタの `$/cancelRequest`（カーソルが動いて不要になった要求の取り消し）に従います。1 リクエストは 15 秒で打ち切り、索引に無い語の全文検索が他の要求を塞がないようにしています。索引の扱いは GUI と同じで、GNU Global があれば索引で引き、無ければ ripgrep に落ちます。

Neovim は追加プラグインなしで接続できます:
...
VSCode は同梱の接続拡張を使います（`editors/vscode/`）。ビルドとインストール:
