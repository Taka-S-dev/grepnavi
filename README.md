...
- **#ifdef 可視化** — C/C++ の条件コンパイルで死ぬブロックをグレーアウト。ヘッダの `#if` ボタンから条件をリストで管理し、真偽の反転・追加のたびに即適用。開いているファイルの `#if` が実際に見ている名前を候補として提示し、ファイルを移っても条件は効き続ける
...
- **デバッグ行の挿入** — カーソル行に printf 等のデバッグ行をテンプレ or 自由入力で挿入し、grepnavi が場所を記録。一覧からジャンプ・書き換え・個別/一括撤去でき、消し忘れは残数バッジで防ぐ。グループ名を付けて撒き分ければ（例: `path-A`）グループ単位でまとめて撤去でき、テンプレの `{group}` で実行出力からも識別できる。挿入行だけをファイルのエンコーディングに変換して書き込むため SJIS/EUC-JP のソースも壊さない（UTF-16 は非対応）。既存行の変更・削除は一切しない。ショートカットは Alt+P、デバッグ行の上では右クリックから書き換え・撤去、`Delete` キーで即撤去、`Tab` / `Shift+Tab` で字下げができる。位置を直したいときは右クリックの「デバッグ行を移動」で移動モードに入り、移動先の行をクリックする（ID を保つので、本文に焼き込まれた `{tag}` と実行時の出力が食い違わない。別ファイルへも移せる）。一覧の ⇅ からはカーソル行へ移せる。個別の撤去と移動は 30 秒以内なら `Ctrl+Z` で元へ戻せる。外部エディタで手動削除・変更した行の記録は、撤去が照合で止まったときに「記録だけ削除」で片付けられる（ファイルには触らない）。
...
- 結果はツリー直下の `.grepnavi-refmap` に保存され、次回からは読み込むだけ（実測: Linux カーネルで初回 87 秒、以後 0.2 秒、ファイルは 20 MB）
...
- **対象から外すもの** — 「設定…」で、このプロジェクトが読まないパスを宣言（`.grepnavi` に保存）。**書き方は `.gitignore` と同じ**で、`!` の打ち消しや `#` のコメントも使える。検索の絞り込みではなくツリーの宣言なので、検索・参照・定義ジャンプ・ファイル一覧・MCP のすべてに効き、検索の glob や「除外ファイルも検索」（Alt+I）では戻らない。tarball を展開して doxygen をかけたツリーでは生成物が同居する（実測: openssl-1.1.1q で `ssl3_read_bytes` を検索すると 208 件中 79 件が `html/` 配下の生成 HTML だった）。**既定は空** — 何が生成物かはツリーごとの事情なので、grepnavi の側から推測して外すことはしない
...
ウィンドウは OS のタイトルバーを持たず、最小化 / 最大化 / 閉じるはページ右上の自前ボタンで行う（バーの空き部分をドラッグで移動、素早い2回押しで最大化）。グラフヘッダ（ノード数・プロジェクト選択・アドオンボタン）もこのバーに統合され、1行ぶん表示が広くなる。従来の OS タイトルバーに戻すには環境変数 `GREPNAVI_NATIVE_TITLEBAR=1` を設定して起動する。
