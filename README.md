...
同じレイアウトは**設定（⚙）の「表示モード」からも選べます**。デスクトップ版の窓にはアドレスバーが無く URL を書き換えられないため、そちらから切り替えてください。選んだモードは次回起動時も維持されます（`?mode=` を付けて開いた窓では URL が優先されます）。

通常モード以外ではエディタペインが無いため、検索結果を開くと外部エディタが起動します（grep だけを grepnavi に任せる使い方）。
