...
- 絞り込みは3タブ横断（スペース区切りは AND、`-` で除外）。パスだけでなく行に付いたシンボル名にも当たるので、関数名で「その関数が絡む行だけ」に絞れます（シンボルは見本 8 件までなので、切れている行は `…` を出し、絞り込み中は取りこぼしうる件数を添えます）。シンボルのチップはクリックで定義へ、Alt+クリックでその参照行を一覧
