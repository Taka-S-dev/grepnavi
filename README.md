...
- **検索結果の絞り込み** — AND（スペース区切り）/ OR（`|`）/ 除外（`-word`）
- **種別バッジ** — 検索ヒット行が関数定義・構造体・`#define`・enum・typedef のどれかを自動判定してバッジ表示
- **検索ディレクトリ指定** — サブディレクトリのみを対象に絞れる
...
- **ラベル・メモ** — ノードごとにラベル（表示名）とメモを編集・保存
...
- **移動ボタン（↑↓←→）** — ノード選択時に詳細パネルに表示。↑↓ で兄弟間を移動、← でレベルアップ（親から切り離す）、→ でレベルダウン（前の兄弟の子にする）
- **キーボードでの移動** — Shift+Alt+Arrow キーで選択ノードを移動
...
4. 詳細パネルでラベル・メモを編集してノードの意味を記録
5. ドラッグ&ドロップ or ↑↓←→ ボタンで親子関係・順序を整理
...
| `Shift+Alt+↑` | 選択ノードを上に移動 |
| `Shift+Alt+↓` | 選択ノードを下に移動 |
| `Shift+Alt+←` | 選択ノードのレベルを上げる（親から切り離す） |
| `Shift+Alt+→` | 選択ノードのレベルを下げる（前の兄弟の子にする） |
...
**移動ボタン**
- ノードを選択すると詳細パネルに ↑ ↓ ← → ボタンが表示される
- または Shift+Alt+Arrow キーで操作
...
└── static/
    ...
    │   ├── project.js         # プロジェクト保存/開く・ルート設定・リサイザー
