...
- **.grepnavi ファイル** — ルートディレクトリに自動生成される設定ファイル。そのルートに紐づく JSON ファイルのリストを保持。複数ポートで起動した場合も同じ `.grepnavi` を共有する
...
| `GET/POST` | `/api/grepnavi` | ルートの `.grepnavi` ファイルの取得/更新（graphs 配列） |
