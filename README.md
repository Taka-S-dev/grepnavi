...
- **シンボル検索パネル** — 左のアクティビティバー（ƒ）から、関数・構造体・マクロを名前のライブサーチで探す。種別チップで「関数だけ」「マクロだけ」に絞り込み、スペース区切りのうろ覚え AND（`recipe save` → `recipe_save`）と 場所（`dir/` / `-test`）と含めるファイル（`*.c,*.h`）の絞り込み欄に対応。ctags 索引が必要（Alt+T のキーボード版と同じバックエンド）
