...
- **デバッグ仕込み** — カーソル行に printf 等のデバッグ行をテンプレ or 自由入力で挿入し、grepnavi が場所を記録。一覧からジャンプ・書き換え・個別/一括撤去でき、消し忘れは残数バッジで防ぐ。グループ名を付けて撒き分ければ（例: `path-A`）グループ単位でまとめて撤去でき、テンプレの `{group}` で実行出力からも識別できる。挿入行だけをファイルのエンコーディングに変換して書き込むため SJIS/EUC-JP のソースも壊さない（UTF-16 は非対応）。既存行の変更・削除は一切しない。ショートカットは Alt+P。
