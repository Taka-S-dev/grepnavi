...
- 結果はツリー直下の `.grepnavi-refmap` に保存され、次回からは読み込むだけ（実測: Linux カーネルで初回 67 秒、以後 0.3 秒、ファイルは 29 MB）
