...
- **ホバープレビュー** — シンボルにマウスを乗せると定義スニペットとその直前コメント（`//` / `/* */` 両形式）をポップアップ表示。GNU Global 対応。`#define FOO BAR` のような別名マクロは参照先を最大4段たどり、実体の値までカードで並べる。`(2|ERR_R_FATAL)` のような定数式マクロは計算値（`= 66 (0x42)`）をヘッダに表示。数値・文字リテラル（`0x42` / `192` / `'A'`）へのホバーは 10進/16進/2進と立っているビット位置を即時表示。右クリックの「基数変換電卓」では `1<<6 | 2` のような式や `(1<<4)==16` の検算に加え、`ERR_R_MALLOC_FAILURE == 65` のようにマクロ名や enum メンバ名を含む式も索引で値を解決して計算できる
