...
export const definitions: ToolDef[] = [
  ...
  },
  {
    ...
    description:
      ...
      "Comment-only and string-only mentions are already filtered out, as are calls inside `#if 0` blocks. A result with an empty `func` and `indirect: true` is a file-scope registration site (a method table or ops struct naming the function); for such functions the registration is the answer to who calls it. Calls reached through function pointers or generated by macros cannot be resolved by any text-based engine, so treat the list as complete-for-direct-calls, not exhaustive.\n\n" +
