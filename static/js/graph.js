...
function renderTree() {
  ...
  const rootSet = Object.values(graph.nodes).filter(
    (n) => !hasParent.has(n.id),
  );
  // hasParent に含まれるノード（子になったノード）は rootOrder から除外する
  const rootOrder = (graph._rootOrder || []).filter(
    (id) => graph.nodes[id] && !hasParent.has(id),
  );
  const roots = rootOrder.length
    ? [
        ...rootOrder.map((id) => graph.nodes[id]),
        ...rootSet.filter((n) => !rootOrder.includes(n.id)),
      ]
    : rootSet;
  ...
  Object.values(graph.nodes).forEach((n) => {
    if (
      hasParent.has(n.id) &&
      !collapsedChildren.has(n.id) &&
      !el.querySelector(`[data-id="${n.id}"]`)
    )
...
function computeDepths(nodes, edges) {
  const depths = {},
    hasParent = new Set(
      (edges || []).filter((e) => e.label !== "seq").map((e) => e.to),
    );
...
function treeLayout(nodeArr, edgeArr) {
  ...
  edgeArr
    .filter((e) => e.label !== "seq")
    .forEach((e) => {
      if (childMap[e.source] !== undefined) childMap[e.source].push(e.target);
    });
  const hasParent = new Set(
    edgeArr.filter((e) => e.label !== "seq").map((e) => e.target),
  );
  ...
  function place(nid, depth) {
    ...
    if (!kids.length) {
      pos[nid] = { x: depth * XG + 90, y: cursor * YG + 50 };
      cursor++;
      placing.delete(nid);
      return;
    }
...
async function renderGraph() {
  ...
  if (!nodeArr.length) {
    container.innerHTML =
      '<div style="color:#555;padding:20px">ノードなし</div>';
    return;
  }
  const edgeArr = (graph.edges || []).map((e) => ({
    source: e.from,
    target: e.to,
    label: e.label,
  }));
  ...
  function nodeById(id_) {
    return nodeArr.find((n) => n.id === id_);
  }
  function edgePath(e) {
  ...
  linkG
    ...
    .attr("marker-end", (e) =>
      e.label === "seq" ? "url(#arr-seq)" : "url(#arr)",
    )
    .attr("d", edgePath);
  ...
  const rootNodes = nodeArr
    .filter((n) => !hasParentSet.has(n.id))
    .sort((a, b) => a.gy - b.gy);
  const autoSeqArr = [];
  ...
  linkG
    ...
    .attr("marker-end", "url(#arr-seq)")
    .attr("d", edgePath);

  const linkLabel = labelG
    .selectAll("text")
    .data(edgeArr.filter((e) => e.label && e.label !== "ref"))
    .enter()
    .append("text")
    .text((d) => d.label)
    .attr("fill", "#888")
    ...
    .attr("x", (d) => {
      const s = nodeById(d.source),
        t = nodeById(d.target);
      return s && t ? (s.gx + NW / 2 + t.gx - NW / 2) / 2 : 0;
    })
    .attr("y", (d) => {
      const s = nodeById(d.source),
        t = nodeById(d.target);
      return s && t ? (s.gy + t.gy) / 2 : 0;
    });
  ...
  const node = nodeG
    ...
    .enter()
    .append("g")
    .attr("class", (d) => "gnode" + (selNode === d.id ? " sel" : ""))
      ...
      d3
        ...
        .on("start", function (e, d) {
          if (e.sourceEvent.shiftKey) {
            ...
            svgPt.x = e.sourceEvent.clientX;
            svgPt.y = e.sourceEvent.clientY;
            const pt = svgPt.matrixTransform(
              root.node().getScreenCTM().inverse(),
            );
            tempLine = root
              .append("line")
        ...
        .on("drag", function (e, d) {
          if (edgeSrc) {
            ...
            const hover = nodeArr.find(
              (n) =>
                n.id !== edgeSrc.id &&
                Math.abs(n.gx - pt.x) < NW / 2 &&
                Math.abs(n.gy - pt.y) < NH() / 2,
            );
            if (hover) {
              nodeG
                .selectAll("g.gnode")
                .filter((n) => n.id === hover.id)
                .select("rect")
                .attr("stroke-width", 3);
            }
          } else {
    ...
    .on("click", (e, d) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        if (graphSel.has(d.id)) graphSel.delete(d.id);
        else graphSel.add(d.id);
      } else {
        graphSel.clear();
        graphSel.add(d.id);
        selectNode(d.id);
  ...
  node
    ...
    .text(
      (d) =>
        shortPath(d.match?.file || "") +
        (d.match?.line ? ":" + d.match.line : ""),
    )
    .attr("text-anchor", "middle")
    .attr("y", 9)
    .attr("font-size", "10px")
  ...
  node.each(function (d) {
    ...
    ng.append("path")
      .attr(
        "d",
        `M0,0 L${SNW - FOLD},0 L${SNW},${FOLD} L${SNW},${SNH} L0,${SNH} Z`,
      )
      .attr("fill", "#fef08a")
      .attr("stroke", "none");
    ng.append("path")
  ...
  function scaleNodes(factor) {
    ...
    nodeArr.forEach((n) => {
      n.gx = cx + (n.gx - cx) * factor;
      n.gy = cy + (n.gy - cy) * factor;
    });
    nodeG
      .selectAll("g.gnode")
      .attr("transform", (d) => `translate(${d.gx},${d.gy})`);
...
async function deleteGraphEdge(from, to) {
  ...
  if (graph.nodes[fromId])
    graph.nodes[fromId].children = (graph.nodes[fromId].children || []).filter(
      (c) => c !== toId,
    );
  renderGraph();
  st("エッジ削除");
...
async function autoConnectSeq() {
  ...
  for (let i = 0; i < roots.length - 1; i++) {
    const from = roots[i].id,
      to = roots[i + 1].id;
    const exists = graph.edges.some(
      (e) =>
        (e.from || e.source) === from &&
        (e.to || e.target) === to &&
        e.label === "seq",
    );
    if (!exists) {
      await addGraphEdge(from, to, "seq");
      count++;
    }
  }
  st(
    count > 0 ? `処理順エッジを ${count} 本追加` : "追加するエッジはありません",
  );
}
...
async function addGraphEdge(fromId, toId, label = "ref") {
  ...
  if (
    label !== "seq" &&
    graph.nodes[fromId] &&
    !graph.nodes[fromId].children?.includes(toId)
  )
    (graph.nodes[fromId].children = graph.nodes[fromId].children || []).push(
      toId,
    );
  renderGraph();
  st("エッジ追加");
...
function attachNodeDragDrop(row, wrap, node) {
  ...
  row.ondragend = (e) => {
    ...
    id("drop-root").style.display = "none";
    const insertBefore = !dropHandled
      ? document.querySelector(".node.insert-before")
      : null;
    const insertAfter = !dropHandled
      ? document.querySelector(".node.insert-after")
      : null;
    document
      .querySelectorAll(".node-row.drag-over,.node-row.indent-target")
      .forEach((el) => {
        el.classList.remove("drag-over", "indent-target");
      });
    document
      .querySelectorAll(".node.insert-before,.node.insert-after")
      .forEach((el) => {
    ...
    if (!dropHandled && e.dataTransfer.dropEffect !== "none") {
      ...
      if (
        targetWrap &&
        targetWrap.dataset.id &&
        targetWrap.dataset.id !== node.id
      ) {
        reorderNode(
          node.id,
          targetWrap.dataset.id,
  ...
  row.ondragover = (e) => {
    ...
    document
      .querySelectorAll(".node-row.drag-over")
  ...
  row.ondrop = (e) => {
    ...
    if (isBefore || isAfter) {
      ...
      if (clientIsDescendant(node.id, movedId, graph.nodes)) {
        st("循環参照になるため移動できません");
        dropHandled = false;
        return;
      }
...
function selectNode(id_) {
  ...
  document
    .querySelectorAll(".children.guide-sel")
    .forEach((el) => el.classList.remove("guide-sel"));
  if (id_) {
...
async function reorderNode(nodeId, refNodeId, position, reorderOnly = false) {
  ...
  if (parentId !== nodeParentId) {
    ...
    if (clientIsDescendant(parentId, nodeId, graph.nodes)) {
      st("循環参照になるため移動できません");
      return;
    }
  ...
  const siblings = parent
    ...
    : (() => {
        ...
        const rootIds = Object.values(graph.nodes)
          .filter((n) => !hasParent.has(n.id))
          .map((n) => n.id);
        const existing = (graph._rootOrder || []).filter(
          (id) => graph.nodes[id],
        );
        const missing = rootIds.filter((id) => !existing.includes(id));
        return [...existing, ...missing];
...
async function addToGraph(match, parentId, edgeLabel, label) {
  ...
  if (parentId && graph.nodes[parentId]) {
    ...
    const hp = new Set();
    Object.values(graph.nodes).forEach((n) =>
      (n.children || []).forEach((c) => hp.add(c)),
    );
    const ordered = (graph._rootOrder || []).filter(
      (id) => graph.nodes[id] && !hp.has(id),
    );
    const unordered = Object.values(graph.nodes)
      .filter((n) => !hp.has(n.id) && !ordered.includes(n.id))
      .map((n) => n.id);
...
function exportGraphDrawio() {
  ...
  for (const n of nodeArr) {
    ...
    const fileLine =
      shortPath(n.match?.file || "") +
      (n.match?.line ? ":" + n.match.line : "");
    const label = xe(
      `<b>${matchText}</b><br><font color="#445566">${fileLine}</font>`,
    );
  ...
  edgeArr.forEach((e, i) => {
    ...
    const label =
      e.label && e.label !== "ref" && e.label !== "seq" ? xe(e.label) : "";
    cells.push(
  ...
  const hasParentSet = new Set(
    edgeArr.filter((e) => e.label !== "seq").map((e) => e.to),
  );
  const rootNodes = nodeArr
    .filter((n) => !hasParentSet.has(n.id))
    .sort((a, b) => (a.gy || 0) - (b.gy || 0));
  for (let i = 0; i < rootNodes.length - 1; i++) {
    const from = rootNodes[i].id,
      to = rootNodes[i + 1].id;
    if (
      !edgeArr.some((e) => e.from === from && e.to === to && e.label === "seq")
    ) {
      cells.push(
  ...
  const xml =
    `<mxfile host="grepnavi"><diagram name="調査グラフ">` +
...
function attachIndentDrag(handle, row, nodeId, depth) {
  ...
  function showDragFeedback(e, newDepth) {
    ...
    if (newDepth < depth) {
      ...
      badge.className = "same";
      arrow.textContent = "─";
    }

    document
      .querySelectorAll(".node-row.indent-target")
      .forEach((el) => el.classList.remove("indent-target"));
    document
      .querySelectorAll(".node.insert-before,.node.insert-after")
      .forEach((el) => {
        el.classList.remove("insert-before", "insert-after");
    ...
    const targetParent =
      newDepth !== depth ? calcDragTarget(nodeId, depth, newDepth) : undefined;
    ...
    if (nodeEl && newDepth !== depth && targetParent !== undefined) {
      ...
      const treeContentTop =
        id("tree").getBoundingClientRect().top - paneRect.top;
      let guideTop;
      if (targetParent) {
        const parentEl = paneEl.querySelector(
          `.node-row[data-id="${targetParent}"]`,
        );
        guideTop = parentEl
    ...
    if (newDepth < depth) {
      ...
      if (currentParent) {
        const parentWrap = id("tree").querySelector(
          `.node[data-id="${currentParent}"]`,
        );
