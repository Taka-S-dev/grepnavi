...
func buildArgs(opts Options) []string {
	...
	if opts.MaxThreads > 0 {
		args = append(args, "--threads", strconv.Itoa(opts.MaxThreads))
	}
	// ctags / gtags の索引ファイルは既定で検索から除外する。tags は巨大な
	// プレーンテキストで、シンボル名でも定義パターン断片でも大量にマッチして
	// ノイズになる (スキャン自体も遅い)。
	// anchored glob (`!/tags`) は、rg が glob を検索ルートではなくカレント
	// ディレクトリ基準で照合するため、cwd の外のルートを検索すると効かない
	// （実測: 同じ glob がツリー内から実行すると効き、外からだと効かない）。
	// そのため unanchored で除外するが、検索 root 直下に「ファイルとして」存在するときに
